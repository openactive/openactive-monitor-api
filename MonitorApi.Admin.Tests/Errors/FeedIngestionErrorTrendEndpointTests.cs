...
/// <summary>
/// Live-data tests for <c>/admin/feed-ingestion-error-trend</c>, including the cross-endpoint invariant
/// that the final trend point agrees with what the incidents endpoint reports for the same day.
/// </summary>
public class FeedIngestionErrorTrendEndpointTests(AdminApiFixture fixture) : IClassFixture<AdminApiFixture>
{
	private const string Route = "/admin/feed-ingestion-error-trend";
	...
	private async Task<AdminPage<IngestionErrorTrendPoint>> Get(string query = "")
	{
		using var client = _fixture.CreateClient();
		var response = await client.GetAsync(_fixture.WithAdminToken(Route + query));

		Assert.Equal(HttpStatusCode.OK, response.StatusCode);
		return (await response.Content.ReadFromJsonAsync<AdminPage<IngestionErrorTrendPoint>>(JsonOptions))!;
	}
	...
	public async Task FinalPointAgreesWithTheIncidentsEndpoint()
	{
		...
		var response = await client.GetAsync(
			_fixture.WithAdminToken("/admin/feed-ingestion-error-incidents?page_size=1000"));
		Assert.Equal(HttpStatusCode.OK, response.StatusCode);
		var incidents = (await response.Content.ReadFromJsonAsync<AdminPage<IngestionErrorIncident>>(JsonOptions))!;
	...
	[Fact]
	public async Task EscalationThresholdEqualToTheFailureThreshold_MakesEveryOpenIncidentPastThreshold()
	{
		var page = await Get("?error_days=1&past_threshold_days=1");
